...
export class EventController {
  ...
  async getAllEvents(@User() user: UserEntity): Promise<EventEntity[]> {
    return this.eventService.getAllEvents(user);
  }

  @Get('count')
  @ApiOperation({ summary: 'Retrieve total events under manager or user' })
  @ApiResponse({
    status: 200,
    description: 'Returns the count of the event.',
    type: Promise<{count: number}>,
  })
  async count(@User() user: UserEntity): Promise<{count: number}> {
    return this.eventService.count(user);
  }
